...
wd_on_katana = '/srv/scratch/z5039045/hospital_effluents'
...
for each in open('urls.txt'):
    if each.startswith('ftp'):
        print(each)
        file_name = each.strip().split('/')[-1]
        print(file_name)
        output_handle = open('%s/qsub_gunzip_%s.sh' % (outputs_folder, file_name), 'w')
        output_handle.write(header)
        output_handle.write(module_lines)
        output_handle.write('cd %s\n' % wd_on_katana)
        output_handle.write('wget %s' % each)
