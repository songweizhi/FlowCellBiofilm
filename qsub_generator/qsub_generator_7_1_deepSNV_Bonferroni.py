...
outputs_folder = 'qsub_deepSNV_Bonferroni'
wd_on_katana = '/srv/scratch/z5039045/Flow_cell_biofilm/4_1_deepSNV_Bonferroni'
...
for sample in open(sample_prefix_file):
    ...
    if sample_group == 'mono_210':
        deepSNV_cmd = 'python3 /srv/scratch/z5039045/Scripts/deep_SNV_runner.py -r ../0_References/2.10wt_illumina.fasta -q ../3_novoalign/%s.bam -c ../3_novoalign/210WTD0.bam' % (sample_id)

    if sample_group == 'mono_D2':
        deepSNV_cmd = 'python3 /srv/scratch/z5039045/Scripts/deep_SNV_runner.py -r ../0_References/D2_pacbio.fasta -q ../3_novoalign/%s.bam -c ../3_novoalign/D2D0.bam' % (sample_id)

    if sample_group == 'coculture':
        deepSNV_cmd = 'python3 /srv/scratch/z5039045/Scripts/deep_SNV_runner.py -r ../0_References/combined_references.fasta -q ../3_novoalign/%s.bam -c ../3_novoalign/coculture_D0.bam' % (sample_id)

    print(deepSNV_cmd)

    output_handle = open('%s/qsub_deepSNV_Bonferroni_%s.sh' % (outputs_folder, sample_id), 'w')
